...
type Transpose<
  M extends number[][],
  RowSize = M["length"] extends 0 ? [] : M[0]
> = {
  [J in keyof RowSize]: {
    [I in keyof M]: J extends keyof M[I] ? M[I][J] : never;
  };
};
...
> = M["length"] extends 0
  ? []
  : J["length"] extends M[number]["length"]
  ? []
  : [Distribute<M, J["length"]>, ...Transpose<M, [...J, true]>];

type Distribute<
  M extends number[][],
  ElementIndex extends number,
  I extends true[] = []
> = I["length"] extends M["length"]
  ? []
  : [
      M[I["length"]][ElementIndex],
      ...Distribute<M, ElementIndex, [...I, true]>
    ];
*/
