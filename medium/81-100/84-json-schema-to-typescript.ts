...
type JSONSchema2TS<T extends Input> = T extends { type: any }
  ? T["type"] extends "boolean"
    ...
    ? // handle object
      T extends { properties: Record<string, Input> }
      ? Omit<
          {
            ...
            [Key in keyof T["properties"] as T extends { required: string[] }
              ? Key extends T["required"][number]
                ? Key
                : never
              : never]: JSONSchema2TS<T["properties"][Key]>;
          },
          never
        >
      : Record<string, unknown>
    : // handle array
    T extends { items: Input }
