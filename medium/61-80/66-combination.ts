...
type Combination<
  T extends string[], // Used only on the first call
  TElements = T[number], // Used instead of T, to make things easier
  TItem = TElements // Used as a forEach
> = TItem extends string
  ? TItem | `${TItem} ${Combination<[], Exclude<TElements, TItem>>}`
  : never;
