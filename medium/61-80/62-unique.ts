...
type Includes<TList extends unknown[], TItem> = TList extends [
  infer IFirst,
  ...infer IRest
]
  ? Equal<IFirst, TItem> extends true
    ? true
    : Includes<IRest, TItem>
  : false;

type Unique<T extends unknown[], TResult extends unknown[] = []> = T extends [
  infer IFirst,
  ...infer IRest
]
  ? Includes<TResult, IFirst> extends true
    ? Unique<IRest, TResult>
    : Unique<IRest, [...TResult, IFirst]>
  : TResult;
