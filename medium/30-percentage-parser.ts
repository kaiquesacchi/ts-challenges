...
/** Used to check if the first char from string is a signal. If not, returns never */
type CheckSignal<T extends string> = T extends "+" | "-" ? T : never;

/** Used to check if the string ends in '%', and splits it from the number */
type CheckPercentage<T extends string> = T extends `${infer INumber}%`
  ? [INumber, "%"]
  : [T, ""];

type PercentageParser<T extends string> = T extends `${CheckSignal<
  infer ISignal
>}${infer IRest}`
  ? [ISignal, ...CheckPercentage<IRest>]
  : ["", ...CheckPercentage<T>];
